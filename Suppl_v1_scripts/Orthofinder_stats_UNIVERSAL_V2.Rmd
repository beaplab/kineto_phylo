...
for (suffix in list) {
  path <- paste0("/home/dzavadska/Data/COMPARATIVE_DATASET/SINGLE_GENE_TREES_2ND_TRY/", suffix, "Orthogroups.GeneCount.tsv")
  df <- fread(path)
...
```{r}
short_suffix <- "V2"
  path <- paste0("/home/dzavadska/Data/COMPARATIVE_DATASET/SINGLE_GENE_TREES_2ND_TRY/V2Orthogroups.GeneCount.tsv")
  df <- fread(path)
...
ggsave("sum_plot_V2.png", sum_plot)
...
```{r}
vocabulary <- fread("/home/zavadska/Data/COMPARATIVE_DATASET_dazhbog/actual_trees_2_0/occ_to_plot.tsv", header = F)
colnames(vocabulary) <- c("missing","species")
vocabulary$fraction_present <- (444-vocabulary$missing)/444
